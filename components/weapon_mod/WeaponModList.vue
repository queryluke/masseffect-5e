      ...
      v-expansion-panel-content(v-for="item in items" v-bind:key="item.id")
        div(slot="header")
          v-layout
            ...
            v-flex(:class="headers[1].classes") {{ item.placement }}
            v-flex(:class="headers[2].classes") {{ item.availability }}
            v-flex(:class="headers[3].classes") {{ item.cost | groupDigits(',') }}
    ...
    data () {
      return {
        headers: [
          ...
          { label: 'Placement', key: 'placement', classes: 'xs6 md3 lg2' },
          { label: 'Weapon Availability', key: 'weaponType', classes: 'hidden-sm-and-down md3 lg2' },
          { label: 'Cost', key: 'cost', classes: 'hidden-md-and-down lg2', sortable: true },
